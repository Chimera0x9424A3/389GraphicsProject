...
public:
	...
	void render(double currentTime) {
        ...
        //Pass in triangle position and color to graphics pipeline
        //glVertexAttrib4fv(0, tripos);
        //glVertexAttrib4fv(1, tricolor);


        //Keep triangle position constantS
        //GLfloat tripos[] = { 0.0, 0.0, 0.0, 1.0f };
        glVertexAttrib4fv(0, tripos);
        //glVertexAttrib4fv(1, tricolor);
	...
	GLuint compile_shaders(void) {
		...
		static const GLchar * vertex_shader_source[] = {
			...
			"void main(void)														    \n"
			"{                                                                          \n"
            "   const vec4 vertices[3] = vec4[3](vec4(  0.95,  -0.95,  0.95,  1.0),     \n"
            "                                    vec4( -0.96,  -0.96,  0.95,  1.0),     \n"
            "                                    vec4(  0.95,   0.95,  0.95,  1.0));    \n"
            "                                                                           \n"
            "   const vec4 color[] = vec4[3](vec4(  1.0,   0.0,  0.0,  1.0),            \n"
            "                                vec4(  0.0,   1.0,  0.0,  1.0),            \n"
            "                                vec4(  0.0,   0.0,  1.0,  1.0));           \n"
            "                                                                           \n"
            "   gl_Position = vertices[gl_VertexID] + offset;                           \n"
            "   t_color = color[gl_VertexID];                                           \n"
            //"   t_color = tricolor;                                                     \n"
        ...
        static const GLchar * tesselation_control_shader_source[] = {
            ...
            "in vec4[gl_MaxPatchVertices] t_color;              \n"
            "out vec4[3] t2color;                               \n"
            "                                                   \n"
            "void main(void)                                    \n"
            "{                                                  \n"
            ...
            "                                                   \n"
            "   t2color[gl_InvocationID] = t_color[gl_InvocationID];      \n"
            "                                                   \n"
            "}                                                  \n"
        };
        ...
        static const GLchar * tesselation_eval_shader_source[] = {
            ...
            "in vec4[gl_MaxPatchVertices] t2color;              \n"
            "out vec4[3] t3color;                                  \n"
            "                                                   \n"
            "void main(void)                                    \n"
            "{                                                  \n"
		...
		static const GLchar * fragment_shader_source[] = {
			...
			"void main(void)                                    \n"
			"{                                                  \n"
            //"      trianglecolor = t_color;                     \n"
            "                                                   \n"
            "   trianglecolor = vec4(sin(gl_FragCoord.x * 0.075) + t_color[0], \n"
            "                sin(gl_FragCoord.y * 0.025) + t_color[1], \n"
            "                sin(gl_FragCoord.x * 0.05) + t_color[2],   \n"
            "                1.0);                               \n"
            //"   trianglecolor = t4color;                        \n"
			"}                                                  \n"
		};
